...
The sources go into the `system` message along with the rules ([#L82](../apps/api/src/chat/prompt.ts#L82)). Models give that message the most authority, so a document's text is read with the same rank as the instructions: a document that says "ignore the rules above" is placed where it may be obeyed. Every document belongs to the reader asking, so the only person who can plant an instruction is the person it would affect, as long as the text is theirs. An imported PDF is where it stops being theirs: its text is whoever wrote the file, hidden text included, and the one defence is that the editor puts the extracted text in the field for the reader to see before it is saved. Sharing would take even that away. A real app keeps only the rules in the `system` message. It passes the sources in a user message, fenced and labelled as data rather than instructions. That reduces the risk without removing it. Moving the sources out also sets up prompt caching (below).
