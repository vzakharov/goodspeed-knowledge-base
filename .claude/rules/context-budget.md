---
description: The context budget hook — what reading it takes after each tool call, when its warning and pause notices fire, and what it stays silent on
paths:
  - .claude/hooks/post-tool-context-budget.sh
  - scripts/context-budget.test.ts
---

# The context budget hook

`.claude/hooks/post-tool-context-budget.sh` tells the agent when its session's
context crosses 200k tokens (a warning) and 300k (the pause), so work is left
resumable before a compact or a dead session takes the choice away. What the
agent does on each notice is `@.claude/skills/go/SKILL.md` § "Stopping partway
releases the plan".
