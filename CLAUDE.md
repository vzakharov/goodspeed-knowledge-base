...
**An audio recording the operator attaches is read with `pnpm transcribe <file>`** — Deepgram, keyed by `DEEPGRAM_API_KEY` from the environment. The transcript is a speech model's hearing of it: product names come out garbled, so restore them from the repo's own vocabulary before acting on one.
