...
- **`pnpm knip` holds every workspace to no unused file, export or dependency.** An export kept for a later step carries a `@tobeused` JSDoc tag, which `knip.json` excludes; any other finding is fixed by deleting the code, or the `export` where the only use is in its own file.
