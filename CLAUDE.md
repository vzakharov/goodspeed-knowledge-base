...
**An audio recording the operator attaches is read with `pnpm transcribe <file>`** — Deepgram (keyed by `DEEPGRAM_API_KEY` from the environment) and a local faster-whisper, merged word by word, the texts saved under `docs/remove-before-merging/transcripts/` for review, so the merge never carries them. Where the two disagree, neither is authoritative: settle it from context and the repo's own vocabulary, and a term both mishear belongs in the script's `VOCABULARY`.
